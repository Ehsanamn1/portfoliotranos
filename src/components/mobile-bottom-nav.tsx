...
export default function MobileBottomNav(){
 ...
 return <nav className="tx-mobile-bottom-nav" aria-label="Mobile navigation">{items.map(([href,label,icon],index)=><a key={href} href={href} className={index===0?"active":""}><span>{icon}</span><small>{label}</small></a>)}</nav>;
}
